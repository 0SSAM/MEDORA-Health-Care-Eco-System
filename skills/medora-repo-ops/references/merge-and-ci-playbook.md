...
| `require_last_push_approval` blocks even admin merge | Push a trivial commit authored by a *different* actor (e.g., `-c user.name="System AI" -c user.email="agent@medora.local"`) so the last pusher differs from the approver; wait for CI; then `gh pr merge --admin --squash --delete-branch`. Alternative: user approves the PR from the web UI (they are not the last pusher), then merge |

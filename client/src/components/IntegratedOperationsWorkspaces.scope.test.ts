...
describe("IntegratedOperationsWorkspaces jurisdiction scope contract", () => {
  ...
  it("treats jurisdiction 0 as a present organization scope", () => {
    expect(hasOrganizationJurisdictionScope(1, 0)).toBe(true);
    expect(hasOrganizationJurisdictionScope(1, null)).toBe(false);
  });

  it("uses the shared zero-safe helper for the scope notice and scoped workspaces", () => {

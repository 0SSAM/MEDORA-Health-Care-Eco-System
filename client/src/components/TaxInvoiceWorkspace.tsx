...
import { appendSafePrintText, openSafePrintWindow } from "@/lib/safePrintDocument";
...
export function TaxInvoiceWorkspace({ branchId, jurisdictionId }: { branchId: number | null; jurisdictionId: number | null }) {
  ...
  const printInvoice = () => {
    ...
    const accentColor = /^#[\da-f]{6}$/i.test(templateValues.accentColor) ? templateValues.accentColor : "#0f766e";
    const paperCss = paperSize === "receipt80" ? "80mm auto" : paperSize;
    const popup = openSafePrintWindow({
      name: "medora-tax-invoice-print",
      title: invoiceNumber,
      direction: "rtl",
      language: "ar",
      features: "width=900,height=900",
      styleText: `@page{size:${paperCss};margin:10mm}body{font-family:'Noto Sans Arabic',Arial,sans-serif;margin:20px;color:#172033}header{border-bottom:4px solid ${accentColor};padding-bottom:18px;display:flex;justify-content:space-between}h1{margin:0;color:${accentColor}}table{width:100%;border-collapse:collapse;margin-top:30px}th,td{border:1px solid #dbe2ea;padding:10px;text-align:right}th{background:#f1f5f9}.totals{margin-top:24px;margin-right:auto;width:280px;line-height:2}.footer{margin-top:48px;border-top:1px solid #dbe2ea;padding-top:16px;color:#475569}`,
    });
    if (!popup) return;
    ...
    appendSafePrintText(document, issuer, "h1", templateValues.nameAr || templateValues.nameEn);
    appendSafePrintText(document, issuer, "div", templateValues.addressAr || templateValues.addressEn);
    appendSafePrintText(document, issuer, "div", templateValues.taxRegistrationNumber);
    appendSafePrintText(document, issuer, "div", [templateValues.phone, templateValues.email].filter(Boolean).join(" "));
    const invoiceMeta = document.createElement("div");
    appendSafePrintText(document, invoiceMeta, "strong", "فاتورة ضريبية محلية");
    appendSafePrintText(document, invoiceMeta, "div", invoiceNumber);
    appendSafePrintText(document, invoiceMeta, "div", new Date().toLocaleString("ar-EG"));
    header.append(issuer, invoiceMeta);
    document.body.append(header);
    const table = document.createElement("table");
    const head = document.createElement("thead");
    const headRow = document.createElement("tr");
    ["الصنف / SKU", "الكمية", "سعر الوحدة", "الضريبة", "الإجمالي"].forEach(value => appendSafePrintText(document, headRow, "th", value));
    head.append(headRow);
    const body = document.createElement("tbody");
    invoiceResult.lines.forEach(line => {
      const row = document.createElement("tr");
      [line.sku, String(line.quantity), numberText(line.unitPrice, invoiceResult.currencyCode), numberText(line.tax, invoiceResult.currencyCode), numberText(line.total, invoiceResult.currencyCode)].forEach(value => appendSafePrintText(document, row, "td", value));
      body.append(row);
    });
    ...
    appendSafePrintText(document, totals, "div", `الإجمالي قبل الضريبة: ${numberText(invoiceResult.subtotal, invoiceResult.currencyCode)}`);
    appendSafePrintText(document, totals, "div", `ضريبة القيمة المضافة: ${numberText(invoiceResult.vatAmount, invoiceResult.currencyCode)}`);
    appendSafePrintText(document, totals, "strong", `الإجمالي: ${numberText(invoiceResult.total, invoiceResult.currencyCode)}`);
    document.body.append(totals);
    appendSafePrintText(document, document.body, "div", `${templateValues.footerAr || templateValues.footerEn}\nETA: الإرسال الرسمي مغلق حتى استكمال الاعتمادات.\nالطابعة المحددة: ${printerLabel}`, "footer");
    popup.focus();
    popup.print();
  };

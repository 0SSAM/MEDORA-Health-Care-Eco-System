...
export function TaxInvoiceWorkspace({ branchId, jurisdictionId }: { branchId: number | null; jurisdictionId: number | null }) {
  ...
  const printInvoice = () => {
    ...
    if (!popup) return;
    const lineRows = invoiceResult.lines.map(line => `<tr><td>${escapeHtml(line.sku)}</td><td>${escapeHtml(String(line.quantity))}</td><td>${escapeHtml(numberText(line.unitPrice, invoiceResult.currencyCode))}</td><td>${escapeHtml(numberText(line.tax, invoiceResult.currencyCode))}</td><td>${escapeHtml(numberText(line.total, invoiceResult.currencyCode))}</td></tr>`).join("");
    ...
    // Sanitize template values used in CSS to prevent injection
    const accentColor = /#[0-9a-fA-F]{3,6}/.test(templateValues.accentColor) ? templateValues.accentColor : "#0f766e";
    popup.document.write(`<!doctype html><html dir="rtl"><head><title>${escapeHtml(invoiceNumber)}</title><style>@page{size:${escapeHtml(paperCss)};margin:10mm}body{font-family:'Noto Sans Arabic',Arial,sans-serif;margin:20px;color:#172033}header{border-bottom:4px solid ${accentColor};padding-bottom:18px;display:flex;justify-content:space-between}h1{margin:0;color:${accentColor}}table{width:100%;border-collapse:collapse;margin-top:30px}th,td{border:1px solid #dbe2ea;padding:10px;text-align:right}th{background:#f1f5f9}.totals{margin-top:24px;margin-right:auto;width:280px;line-height:2}.footer{margin-top:48px;border-top:1px solid #dbe2ea;padding-top:16px;color:#475569}</style></head><body><header><div><h1>${escapeHtml(templateValues.nameAr || templateValues.nameEn)}</h1><div>${escapeHtml(templateValues.addressAr || templateValues.addressEn)}</div><div>${escapeHtml(templateValues.taxRegistrationNumber)}</div><div>${escapeHtml(templateValues.phone)} ${escapeHtml(templateValues.email)}</div></div><div><strong>فاتورة ضريبية محلية</strong><br/>${escapeHtml(invoiceNumber)}<br/>${escapeHtml(new Date().toLocaleString("ar-EG"))}</div></header><table><thead><tr><th>الصنف / SKU</th><th>الكمية</th><th>سعر الوحدة</th><th>الضريبة</th><th>الإجمالي</th></tr></thead><tbody>${lineRows}</tbody></table><div class="totals"><div>الإجمالي قبل الضريبة: ${escapeHtml(numberText(invoiceResult.subtotal, invoiceResult.currencyCode))}</div><div>ضريبة القيمة المضافة: ${escapeHtml(numberText(invoiceResult.vatAmount, invoiceResult.currencyCode))}</div><strong>الإجمالي: ${escapeHtml(numberText(invoiceResult.total, invoiceResult.currencyCode))}</strong></div><div class="footer">${escapeHtml(templateValues.footerAr || templateValues.footerEn)}<br/>ETA: الإرسال الرسمي مغلق حتى استكمال الاعتمادات.<br/>الطابعة المحددة: ${escapeHtml(printerLabel)}</div></body></html>`);

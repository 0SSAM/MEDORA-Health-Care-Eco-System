...
export const authenticationEvents = mysqlTable("authentication_events", {
  ...
  branchId: int("branchId"),
  jurisdictionId: int("jurisdictionId"),
  eventType: mysqlEnum("eventType", ["login_success", "login_failure", "logout", "lockout", "session_revoked", "password_reset_requested", "password_reset_completed", "cache_refreshed"]).notNull(),
...
// RBAC + delivery tables required by scripts/seed-rbac-and-roles.mjs and
// scripts/seed-delivery-zones.mjs (snake_case columns to match the seeds).
...
}, table => ({ codeIdx: uniqueIndex("rbac_permissions_code_idx").on(table.code) }));
...
}, table => ({ orgCodeIdx: uniqueIndex("rbac_roles_org_code_idx").on(table.organizationId, table.code) }));

export const rbacRolePermissions = mysqlTable("rbac_role_permissions", {
  id: int("id").autoincrement().primaryKey(),
  roleId: int("role_id").notNull(),
  permissionId: int("permission_id").notNull(),
}, table => ({ rolePermIdx: uniqueIndex("rbac_role_permissions_uq").on(table.roleId, table.permissionId) }));
...
export const rbacUserRoles = mysqlTable("rbac_user_roles", {
  ...
  roleId: int("role_id").notNull(),
}, table => ({ orgUserRoleIdx: uniqueIndex("rbac_user_roles_uq").on(table.organizationId, table.userId, table.roleId) }));
...
export const deliveryZones = mysqlTable("delivery_zones", {
  ...
  deliveryTimeMin: int("delivery_time_min").notNull().default(60),
  active: int("active").notNull().default(1),
}, table => ({ orgNameIdx: uniqueIndex("delivery_zones_org_name_idx").on(table.organizationId, table.nameAr) }));
...
export const deliveryDrivers = mysqlTable("delivery_drivers", {
  ...
  vehicleType: varchar("vehicle_type", { length: 40 }).notNull().default("motorcycle"),
  status: varchar("status", { length: 40 }).notNull().default("available"),
  active: int("active").notNull().default(1),
}, table => ({ orgPhoneIdx: uniqueIndex("delivery_drivers_org_phone_idx").on(table.organizationId, table.phone) }));

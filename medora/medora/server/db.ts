...
import { InsertUser, authenticationEvents, internalCredentials, internalSessions, passwordResetTokens, users, ndaAcceptances, organizationMemberships, branchUsers, branches, branchJurisdictions, organizations, jurisdictionProfiles, healthcarePatients, ePrescriptions, ePrescriptionLines } from "../drizzle/schema";
...
export async function getInternalSession(token: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select({ session: internalSessions, user: users }).from(internalSessions)

...
import { addCashSale, beginCashShiftClose, closeCashShift, openCashShift, recordCashMovement } from "./cash-shift-policy";

describe("cash shift policy", () => {
  it("supports opening, sales, movements, closing, and a balanced close", () => {
    let shift = { id: 1, ...openCashShift({ organizationId: 10, branchId: 20, openedByUserId: 7, openingFloat: 100 }) };
  ...
  it("requires independent approval for a cashier variance", () => {
    let shift = { id: 2, ...openCashShift({ organizationId: 10, branchId: 20, openedByUserId: 7, openingFloat: 100 }) };
    shift = addCashSale(shift, 250);
    shift = beginCashShiftClose(shift, 7);
  ...
  it("does not accept sales after closing", () => {
    let shift = { id: 3, ...openCashShift({ organizationId: 10, branchId: 20, openedByUserId: 7, openingFloat: 50 }) };
    shift = beginCashShiftClose(shift, 7);

...
async function provisionAdmin(user,pass){
  ...
  const jId=(await one("SELECT id FROM jurisdiction_profiles WHERE countryCode='EG' LIMIT 1"))?.id || await insert("jurisdiction_profiles",{countryCode:(jC.includes("EG")?"EG":jC[0]||"EG"),countryNameAr:"مصر",defaultLocale:"ar",currencyCode:(jCur.includes("EGP")?"EGP":jCur[0]||"EGP"),timezone:"Africa/Cairo",taxProfile:"STANDARD",dateFormat:"YYYY-MM-DD",numberSystem:"latn",active:1});
  ...
  const locTypes=(await enumOf("branch_jurisdictions","locationSource"))||[];
  if(!(await one("SELECT id FROM branch_jurisdictions WHERE branchId=?",[brId]))) await insert("branch_jurisdictions",{branchId:brId,jurisdictionId:jId,locationSource:(locTypes.includes("admin_confirmed")?"admin_confirmed":locTypes[0]||"admin_confirmed"),confirmedByUserId:userId});

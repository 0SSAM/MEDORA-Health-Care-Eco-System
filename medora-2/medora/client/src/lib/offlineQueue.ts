...
/** Production offline persistence is IndexedDB-only; Web Storage is never used for draft payloads. */
export async function enqueueOfflineDraft(draft: Omit<OfflineDraft, "id" | "createdAt">): Promise<OfflineDraft> {
...
function isStoredDraftSafe(item: unknown): item is OfflineDraft {
  ...
  return draft.regulated === false && typeof draft.id === "string" && typeof draft.idempotencyKey === "string" && isOfflinePayloadSafe(draft.payload);
}

export async function listOfflineDrafts(): Promise<OfflineDraft[]> {
  return listDurableOfflineDrafts();
}
...
export async function listDurableOfflineDrafts(): Promise<OfflineDraft[]> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      cleanup.transaction.onerror = () => { db.close(); reject(cleanup.transaction.error ?? new Error("draft-cleanup-failed")); };
    };
...
export async function updateOfflineDraft(id: string, patch: Pick<OfflineDraft, "status" | "conflictReason" | "lastError" | "lastAttemptAt">): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    get.onsuccess = () => {
      if (!get.result || !isStoredDraftSafe(get.result)) {
        reject(new Error("draft-not-found-or-unsafe"));
        return;
      }
      store.put({ ...get.result, ...patch });
    };
    get.onerror = () => reject(get.error ?? new Error("draft-update-read-failed"));

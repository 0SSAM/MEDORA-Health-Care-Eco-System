...
describe("internal request cookie parsing", () => {
  it("parses the internal session cookie before tRPC context creation", () => {
    ...
    const req = { headers: { cookie: `theme=light; aldo_internal_session=${token}` } };

    attachRequestCookies(req);

    expect(req.cookies).toEqual({
      theme: "light",
      aldo_internal_session: token,
    });
    expect(readCookie(req.headers.cookie, "aldo_internal_session")).toBe(token);
  });
  ...
  it("returns an empty object for absent or malformed headers without throwing", () => {
    ...
    expect(readCookie("theme=light", "aldo_internal_session")).toBeNull();
  });
});

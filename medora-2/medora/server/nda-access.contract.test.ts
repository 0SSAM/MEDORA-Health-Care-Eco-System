...
describe("MEDORA NDA access-gate contract", () => {
  ...
  it("blocks protected and admin data procedures until both the current NDA version and hash are accepted", () => {
    ...
    expect(trpc).toContain("export const adminProcedure = t.procedure.use(");
    expect(trpc).toContain("if (!ctx.user || ctx.user.role !== 'admin')");
    expect(trpc).toContain(").use(requireCurrentNda);");
    expect(trpc).not.toMatch(/blockShowcaseMutations|isolatedDemoMutationProcedure|showcase/iu);
  });

...
export default defineConfig(({ mode }) => {
  const isCloudflarePreview = mode === "cloudflare-preview";

  // Cloudflare preview must be a clean browser-only build. Manus runtime/debug
  // plugins are intentionally excluded because they are development-environment
  // integrations and can inject runtime assumptions unavailable on Workers.
  const plugins = [react(), tailwindcss(), ...(isCloudflarePreview ? [] : [jsxLocPlugin(), vitePluginManusRuntime(), vitePluginManusDebugCollector()])];

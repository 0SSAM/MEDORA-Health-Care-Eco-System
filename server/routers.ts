...
import { createPasswordResetToken, ensureShowcaseAccount, seedShowcaseDemoData, getInternalCredentialByUsername, getInternalScopeForUser, getInternalSessionModeAvailability, switchInternalSessionMode, createInternalSession, recordAuthenticationEvent, resetInternalPasswordWithToken, revokeInternalSession, getUserById } from "./db";
...
import { isIsolatedShowcaseAdministrator } from "./domain/showcase-admin-policy";
...
export const appRouter = router({
  ...
  auth: router({
    ...
    sessionModes: publicProcedure.query(async ({ ctx }) => {
      if (!ctx.user) return { authenticated: false as const, currentMode: null, showcase: false, production: false };
      const token = ctx.req.cookies?.[INTERNAL_SESSION_COOKIE];
      const availability = token ? await getInternalSessionModeAvailability(token) : undefined;
      return availability ? { authenticated: true as const, ...availability } : { authenticated: false as const, currentMode: null, showcase: false, production: false };
    }),
    switchSessionMode: publicProcedure.input(z.object({ targetMode: z.enum(["production", "showcase"]) })).mutation(async ({ ctx, input }) => {
      if (!ctx.user) return { success: false as const, reason: "unauthenticated" as const };
      const token = ctx.req.cookies?.[INTERNAL_SESSION_COOKIE];
      if (!token) return { success: false as const, reason: "unauthenticated" as const };
      const result = await switchInternalSessionMode(token, input.targetMode);
      if (!result) return { success: false as const, reason: "unavailable" as const };
      return result;
    }),
    sessionInfo: publicProcedure.query(({ ctx }) => ctx.user ? {
      authenticated: true as const,
      accountType: ctx.internalSession?.session.sessionMode === "showcase" ? "showcase" as const : "employee" as const,
      sessionMode: ctx.internalSession?.session.sessionMode ?? "production" as const,
      role: ctx.user.role,
      showcaseAdmin: isIsolatedShowcaseAdministrator({ openId: ctx.user.openId, sessionMode: ctx.internalSession?.session.sessionMode }),
    ...
    internalLogin: publicProcedure.input(z.object({ username: z.string().min(3).max(80), password: z.string().min(1).max(200) })).mutation(async ({ ctx, input }) => {
      ...
      if (credential.accountType === "showcase" && scope) {
        try {
          await seedShowcaseDemoData({ organizationId: scope.organizationId, branchId: scope.branchId, jurisdictionId: scope.jurisdictionId, createdByUserId: credential.userId });
        } catch (error) {
          // Synthetic Demo fixtures are optional presentation data. Never turn a transient
          // seed/lock/duplicate failure into an authentication failure or lose the scoped session.
          console.error("[Auth] showcase fixture bootstrap deferred:", safeErrorLabel(error));
        }
      }
      if (!scope) {
      ...
      return { success: true as const, mode: "internal" as const, scope, role: applicationRole, organizationRole: scope.role, accountType: credential.accountType, sessionMode: credential.accountType === "showcase" ? "showcase" as const : "production" as const };
      } catch (error) {

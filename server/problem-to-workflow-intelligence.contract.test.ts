...
describe("problem-to-workflow intelligence contract", () => {
  it("keeps the guidance loop attached only to the authorized management overview and approved primary action", () => {
    expect(homeSource).toContain('user && active === "overview" && canSeeManagementSurfaces && <OperationalWorkflowGuide');
    expect(homeSource).toContain("const canSeeManagementSurfaces = isManagementRole && hasCompleteOperationalScope;");
    expect(homeSource).toContain("primaryActionLabel={primaryOverviewAction?.label}");
    expect(homeSource).toContain('onOpenPrimaryAction={() => { if (primaryOverviewAction) activateShortcut(primaryOverviewAction.module); }}');

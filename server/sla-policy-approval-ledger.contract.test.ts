...
describe("SLA policy and approval ledger contract", () => {
  ...
  it("keeps procurement list, creation, and reporting jurisdiction-safe with a positive legal jurisdiction", () => {
    expect(procurement).toContain("jurisdictionId: z.number().int().positive().nullable().optional()");
    expect(procurement).toContain("const purchaseOrderScopeSchema = scopeSchema.extend({ jurisdictionId: z.number().int().positive() })");
    ...
    expect(procurement).toContain("reportQuerySchema.extend({ jurisdictionId: z.number().int().positive() })");
  });
  ...
  it("wires the existing purchase-order transition into a manager-only supply review surface", () => {
    ...
    const supplyChainBindings = home.match(/<SupplyChainWorkspace[\s\S]*?\/>/g) ?? [];
    expect(supplyChainBindings.length).toBeGreaterThan(0);
    expect(supplyChainBindings.every(binding => binding.includes("organizationId={organizationId}"))).toBe(true);
    expect(supplyChainBindings.every(binding => binding.includes("branchId={branchId}"))).toBe(true);
    expect(supplyChainBindings.every(binding => binding.includes("jurisdictionId={jurisdictionId}"))).toBe(true);
    expect(supplyChainBindings.every(binding => binding.includes("canManage={canManagePolicies}"))).toBe(true);
  });
});

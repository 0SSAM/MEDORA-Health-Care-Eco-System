...
class SDKServer {
  ...
  /**
   * Create a session token for a Medora user openId
   * @example
  ...
  async verifySession(
    ...
    try {
      ...
      console.warn("[Auth] Session verification failed", safeErrorLabel(error));
      return null;
    }
  }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      try {
        ...
        console.error("[Auth] Failed to sync user from OAuth:", safeErrorLabel(error));
...
function buildCronUser(
  ...
  return {
    id: -1,
    openId: userInfo.openId,
    name: userInfo.name || "System Scheduled Task",
    email: null,
    loginMethod: null,
    role: "user",
    createdAt: now,

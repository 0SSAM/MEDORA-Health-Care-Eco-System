...
function positiveJurisdictionExpenseDb() {
  const selectResults = [
...
describe("financial organization-membership guard", () => {
  ...
  it("permits an authorized financial expense only for a positive legal jurisdiction", async () => {
    const db = positiveJurisdictionExpenseDb();
    getDbMock.mockResolvedValue(db);
    ...
    await expect(caller.erp.accounting.createExpense({
      ...
      title: "صيانة دورية",
      justification: "صيانة دورية موثقة للأصل التشغيلي ضمن النطاق المعتمد.",
    })).resolves.toEqual({ expenseId: 1, status: "pending_review" });

    expect(db.insertedValues[0]).toMatchObject({ organizationId: 10, branchId: 3, jurisdictionId: 7 });
  });
});

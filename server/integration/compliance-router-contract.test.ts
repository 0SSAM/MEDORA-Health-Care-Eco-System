...
const staffUser: TestUser = { id: 81, openId: "compliance-contract-user", email: "compliance-contract@example.com", name: "Compliance Contract User", loginMethod: "manus", role: "manager", createdAt: new Date(0), updatedAt: new Date(0), lastSignedIn: new Date(0) };
function contextFor(user: TestUser): TrpcContext { return { user, req: { protocol: "https", headers: {} } as TrpcContext["req"], res: {} as TrpcContext["res] }; }
...
describe("regional compliance protected router contracts", () => {
  ...
  it("runs the protected pack lifecycle through audit listing with scoped mutations", async () => {
    ...
    const packReview = [{ id: 44, jurisdictionId: 3, status: "review", rulesJson: JSON.stringify(completeRules), effectiveFrom: new Date("2026-01-01"), reviewDueAt: FUTURE_REVIEW_DUE_AT }];
    const packApproved = [{ ...packReview[0], status: "approved" }];
    const packRolledBack = [{ ...packReview[0], status: "rolled_back" }];
    const evidenceReview = [{ id: 9, packId: 44, jurisdictionId: 3, status: "review", ruleKey: "tax", verificationStatus: "review" }];
    ...
    const db = {
      select: vi.fn(() => ({ from: vi.fn(() => ({ where: vi.fn(() => {
        selectCall += 1;
        const rows = selectCall === 1 ? profile : selectCall === 2 ? packReview : selectCall === 3 ? evidenceReview : selectCall === 4 ? packReview : selectCall === 5 ? completeEvidence : selectCall === 6 ? packApproved : selectCall === 7 ? packRolledBack : audit;
        return queryResult(rows);
      }) })) })),
      insert: vi.fn(() => ({ values: vi.fn(async () => [{ insertId: 44 }]) })),

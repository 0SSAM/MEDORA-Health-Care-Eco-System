...
export type Stocktake = { id: number; organizationId: number; branchId: number; createdByUserId: number; status: StocktakeStatus; lines: StocktakeLine[] };

export function startStocktake(input: { id: number; organizationId: number; branchId: number; createdByUserId: number; productIds: number[]; expectedQuantities: number[] }): Stocktake {
  ...
  return { id: input.id, organizationId: input.organizationId, branchId: input.branchId, createdByUserId: input.createdByUserId, status: "counting", lines: input.productIds.map((productId, index) => ({ productId, expectedQuantity: input.expectedQuantities[index] })) };
}

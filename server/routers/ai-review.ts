 ...
 *
 * كل مراجعة تعمل داخل نطاق المنظمة المستهدف. لا يجوز لتقرير AI أن يستنتج
 * مؤشرات من سجلات منظمة أخرى أو أن يسمح لعضو عادي بإغلاق توصيات إدارية.
 */
...
async function countRows(db: any, table: string, organizationId: number): Promise<number> {
  const safe = table.replace(/[^a-z_]/g, "");
  // Every table in MODULE_CHECKS is organization-owned. Never fall back to a global count.
  const rows = (await db.execute(
    sql`SELECT COUNT(*) AS n FROM ${sql.raw(safe)} WHERE organizationId=${organizationId}`
  )) as any;
  return Number(rows?.[0]?.[0]?.n ?? 0);
}

function assertManagementRole(role: string) {
  if (!["admin", "owner", "org_admin", "operations_manager"].includes(role)) {
    throw new TRPCError({ code: "FORBIDDEN", message: "صلاحية إدارة المراجعة مطلوبة." });
  }
}
...
export const aiReviewRouter = router({
  runAutoReview: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const c of MODULE_CHECKS) {
        let n = 0, err = "";
        try { n = await countRows(db, c.table, input.organizationId); } catch (e) { err = String((e as Error).message).slice(0, 140); }
        ...
        await db.execute(sql`INSERT INTO ai_review_findings (run_id, module, kpi, severity, score, message_ar, evidence)
          VALUES (${runId}, ${c.module}, ${c.kpi}, ${severity}, ${score}, ${message}, ${err || `${c.table} count=${n}`})`);
        lines.push(`| ${c.module} | ${c.kpi} | ${status} | ${score}/100 |`);
        overall += score;
        anyFinding = true;
        if (!err && n === 0) {
          await db.execute(sql`INSERT INTO ai_review_recommendations (run_id, priority, title_ar, detail_ar, module, status)
      ...
      await db.execute(sql`UPDATE ai_review_runs SET status='completed', score_overall=${overall}, report_markdown=${report}, finished_at=CURRENT_TIMESTAMP WHERE id=${runId}`);
  ...
  latestReview: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const db = await dbWithOrgScope(ctx as any, input.organizationId);
      const rows = (await db.execute(sql`SELECT id, \`trigger\`, status, score_overall, report_markdown, started_at, finished_at FROM ai_review_runs
        WHERE organization_id=${input.organizationId} ORDER BY id DESC LIMIT 1`)) as any;
      const r = rows?.[0]?.[0];
      if (!r) return null;
  ...
  listReviews: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const db = await dbWithOrgScope(ctx as any, input.organizationId);
      const rows = (await db.execute(sql`SELECT id, \`trigger\`, status, score_overall, started_at, finished_at FROM ai_review_runs
        WHERE organization_id=${input.organizationId} ORDER BY id DESC LIMIT ${input.limit}`)) as any;
      return (rows?.[0] ?? []).map((r: any) => ({ id: r.id, trigger: r.trigger, status: r.status, scoreOverall: r.score_overall, startedAt: r.started_at, finishedAt: r.finished_at }));
    }),
  ...
  resolveRecommendation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      assertManagementRole(ctx.user.role);
      const db = await dbWithOrgScope(ctx as any, input.organizationId);
      const rows = (await db.execute(sql`SELECT r.organization_id
        FROM ai_review_runs r JOIN ai_review_recommendations rec ON rec.run_id=r.id
        WHERE rec.id=${input.id} AND r.organization_id=${input.organizationId} LIMIT 1`)) as any;
      const runOrg = rows?.[0]?.[0]?.organization_id;
      if (!runOrg || Number(runOrg) !== input.organizationId) throw new TRPCError({ code: "NOT_FOUND" });

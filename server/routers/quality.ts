...
function assertScope(ctx: any, input: z.infer<typeof scope>) {
...
export const qualityRouter = router({
  list: protectedProcedure.input(scope).query(async ({ ctx, input }) => {
    ...
    const [rows] = await pool.query(
      `SELECT id, warehouseId, itemCode, batchNo, sampleSize, acceptedUnits, rejectedUnits,
              status, disposition, inspectorUserId, approvedByUserId, reason, createdAt, updatedAt
         FROM quality_inspections
        WHERE organizationId=? AND branchId=? AND jurisdictionId=?
        ORDER BY id DESC LIMIT 200`,
      [input.organizationId, input.branchId, input.jurisdictionId],
    );
    return { inspections: rows };
  }),

  createInspection: protectedProcedure.input(scope.extend({
    ...
    if (!(warehouseRows as Array<{ id: number }>).length) throw new TRPCError({ code: "FORBIDDEN", message: "المخزن خارج نطاق الفرع أو غير نشط." });
    const [result] = await pool.query(
      `INSERT INTO quality_inspections
        (organizationId,branchId,jurisdictionId,warehouseId,itemCode,batchNo,sampleSize,inspectorUserId,reason)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [input.organizationId, input.branchId, input.jurisdictionId, input.warehouseId, input.itemCode, input.batchNo ?? null, input.sampleSize, ctx.user.id, input.reason ?? null],
    );
  ...
  recordResult: protectedProcedure.input(scope.extend({
    inspectionId: z.number().int().positive(),
    acceptedUnits: z.number().int().min(0),
